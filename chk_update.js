...
var yuka_url = 'http://www.earlywing.co.jp/category/blog/';
...
// 3. アーリーウィングブログのスクレイピング
function checkEarlyWing(callback) {
    setTimeout(function () {
        client.fetch(yuka_url, function (err, $, res, body) {
            if (!err) {
                yuka[0] = $('.blogMain h2').eq(0).text().trim();
                yuka[1] = $('.blogMain h2').eq(0).find('a').url();
